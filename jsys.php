...
//пароль для входа (для авторизации нужны включенные cookies)
$JSYS_PASSWORD    = 'joom.ru';
//нужна ли авторизация, 1 - нужна, 0 - не нужна, пароль требоваться не будет
$JSYS_ENABLE_AUTH = 0;
//показывать или нет ссылку принудительной закачки файла, по умолчанию нет (0)
$JSYS_SHOW_DOWNLOAD_LINK = 0;
...
class JSYSOPERATION_Login {
	...
	function main() {
		...
		$form = new JSYS_Form('DoLogin',"Авторизация");
		$form->row('Пароль доступа',"<input type=text name='pass' value=''>");
		$form->out();
	}
	...
	function DoLogin() {
		...
		if ($pass!=$JSYS_PASSWORD) {
			...
			echo "Ошибка: Вы ввели не правильный пароль<br>";
			echo "<br><a href='jsys.php?action=Login&task=main'>Вернуться к форме входа</a>";
			return;
		}
		
		setcookie($COOKIE_NAME,md5($JSYS_PASSWORD));
		
		echo "Вы вошли в систему<br>";
	}
...
class JSYSOPERATION_Main {
	...
	function main() {
		...
		echo "Утилиты, полезные для джумлы. Автор набора - Dead Krolik. Дизайн сперт с oswd.org (я в курсе что это плохой дизайн с точки зрения нормальный дизайнеров, но я к таковым не принадлежу), автор дизайна Leandro Pereira. Содержит в себе библиотеку pclZip (http://www.phpconcept.net). Содержит в себе класс работы с бд от Joomla. Распространяется бесплатно. ";
	}
...
class JSYSOPERATION_SetPassword {
	...
	function main() {
		...
		if (!file_exists($conf)) {
			...
			echo "Файл конфигурации Joomla не найден. Не возможно подключиться к базе данных.";
			return;
		}
		...
		$form = new JSYS_Form('MakePass','Смена пароля пользователя');
		$form->row('Выберите пользователя',$sel);
		$form->row('Новый пароль',"<input type=text name='new_password'>");
		$form->out();
	}
	...
	function MakePass() {
		...
		if (!$npass || !$uid) {
			...
			echo "Не правильный пользователь или пустой пароль";
			return;
		}
...
class JSYSOPERATION_FileView {
	...
	function viewDir() {
	    ...
	    if ($dir===false) {
	    	...
	    	echo "Не могу открыть директорию ".$this->base.$DIR."<br>";
	    	return;
	    }
	    ...
	    for ($i=0,$start = $outdir_a[0];$i<sizeof($outdir_a)-1;$i++) {
	    	...
	    	$outdir = str_replace($di,"<a href='jsys.php?action=FileView&DIR=$dlink'>".$di."</a>",$outdir);
	    }
	    
		echo "<p align=center>Содержание директории <b>{$this->base}{$outdir}</b></p>
		<form action='jsys.php?action=FileView' method=post>
		<table align=center width=100%>
		<tr><td width=20><input type=checkbox name=mainchk onClick='checkAll(".sizeof($files).")'></td><td width=80%>Имя</td><td width=18%>Размер</td><td>Права</td><td>Владелец</td></tr>";
		...
		$myuid = getmyuid();
		if ($DIR!='/') echo "<tr><td width=20></td><td width=80%><a href='jsys.php?action=FileView&DIR=".urlencode($parent)."'><b>..</b></a></td><td width=18%></td><td></td><td></td></tr>";
		...
		foreach($files as $file) {
			...
			if (is_dir($file)) {
				...
				$name = "<a href='jsys.php?action=FileView&DIR=".urlencode($DIR.$bname.'/')."'><b>$bname</b></a>";
				$stat = '';
			}
			else {
				...
				$name = "<a href='jsys.php?action=FileView&task=ViewFile&File=".urlencode($file)."&DIR=".urlencode($DIR)."'>$rname</a>";

				global $JSYS_SHOW_DOWNLOAD_LINK;
				if ($JSYS_SHOW_DOWNLOAD_LINK) $name .= "&nbsp;<a href='jsys.php?action=FileView&task=ReadFile&NO_TEMPLATE=1&src=".urlencode($file)."'>+</a>";
			...
			$owner = $myuid != fileowner($file) ? 'Скрипт' : 'Юзер';
		...
		<ul>
			<li>
			<nobr>[ <a onClick=\"dosbm('Archive')\" href='#'>архивировать в</a> <input type=text name='sel_file_name' size=8 value='tmp.zip'> ]</nobr>
			<nobr>[ <a onClick=\"javascript:if(confirm('Вы точно хотите разархивировать?')) { dosbm('UnArchive'); }\" href='#'>разархивировать</a> ]</nobr>
			<nobr>[ <a onClick=\"dosbm('AddToArchive')\" href='#'>добавить к архиву</a> <input type=text size=8 name=archive value='{$last_zip}'> ]</nobr>
			</li>
			<li>
			<nobr>[ <a onClick=\"javascript:if(confirm('Вы точно хотите удалить?')) { dosbm('Delete'); }\" href='#'>удалить</a> ]</nobr>
			<nobr>[ <a onClick=\"dosbm('Rename')\" href='#'>переименовать</a> ]</nobr>
			<nobr>[ <a onClick=\"dosbm('Move')\" href='#'>переместить/скопировать</a> ( глубина <input type=text name='deep' size=2 value='1'> ) ]</nobr>
			</li>
			<li>
			<nobr>[ <a onClick=\"dosbm('DownloadFile')\" href='#'>скачать удаленный файл</a> ]</nobr>
			</li>
			<li>
			<nobr>[ <a onClick=\"dosbm('Chmod')\" href='#'>сменить права</a> ]</nobr>
			</li>

		</ul>	
		</td></tr>
		<!-- <tr><td colspan=3 align=center><input type=submit value='Отправить'></td></tr> -->
	...
	function ViewFile() {
		...
		switch($ext) {
			...
			case 'zip':
				...
				echo "<p align=center>Содержимое архива <b>$file</b></p><table align=center width=100%>";
				foreach($c as $f) {
				
					echo "<tr><td>".$f['filename']."</td><td>".($f['folder'] ? 'Папка' : 'Файл')."</td></tr>";
				}
				echo "</table><p align=center><input type=button onClick=\"window.history.go(-1);\" value='Вернуться'></p>";
				break;
			...
			case 'jpeg':
			case 'png':
				echo "<p style='text-align: center; background: #ffffff; padding:15px'><img src='jsys.php?action=FileView&task=ViewImage&NO_TEMPLATE=1&src=".urlencode($file)."'></p>";
				echo "<p align=center><input type=button onClick=\"window.history.go(-1);\" value='Вернуться'></p>";
				break;
				
			default:
				echo "Файл: <b>$file</b> [для записи: <b>".(is_writable($file) ? '<font color=green>доступен</font>' : '<font color=red>недоступен</font>')."</b>]
				<form action='jsys.php?action=FileView&task=SaveFile' method=post>
				<p align=center><textarea name=content cols=100 rows=30>".htmlspecialchars(file_get_contents($file))."</textarea></p>
				<input type=hidden name=file value=\"$file\">
				<input type=hidden name=DIR value=\"$DIR\">
				<p align=center><input type=checkbox value=1 name=do_bak checked>Сделать резервную копию перед сохранением</p>
				<p align=center><input type=button onClick=\"window.history.go(-1);\" value='Отменить'><input type=submit value='Сохранить'></p>
				</form>";
				break;
		}
	}
	...
	function SaveFile() {
		...
		if ($f) {
			...
			if ($do_bak) {
				...
				@fwrite($f,$c);
				@fclose($f);
				echo "Создание резервной копии ".($res ? 'OK' : 'ERROR').' ('.basename($file.'.bak').')<br>';
			}
		}
		else {
		
			echo "Не могу открыть файл для записи ERROR<br>";
		}

		echo "<br><a href='jsys.php?action=FileView&DIR=".urlencode($DIR)."'>Вернуться к директории</a>";
	}
	...
	function Archive() {
		...
		if (sizeof($ofiles)==0) {
			...
			echo "Выберите файлы для архивации";
			return;
		}
		...
		if (!is_writable(dirname($ar_file))) {
			...
			echo "Директория ".dirname($ar_file).' недоступна для записи';
			return;
		}
		...
		if ($v_list == 0) {
			...
			echo "Ошибка архивации : ".$archive->errorInfo(true);
		}
		else {

			echo "Архив успешно создан, его можно <a href='".substr($DIR,1).basename($ar_file)."'>[скачать]</a>";
		}
	...
	function UnArchive() {
		...
		if (sizeof($ofiles)==0) {
			...
			echo "Выберите файлы для разархивации";
			return;
		}
		...
		foreach($ofiles as $file) {
			...
			if ($ret == 0) {
				...
				echo "Ошибка архивации : ".$archive->errorInfo(true).'<br>';
			}
			else {
			
				echo "Архив $file успешно распакован<br>";
			}
		}
	...
	function AddToArchive() {
		...
		if ($ar_file=='') {
			...
			echo "Введите имя архива, в который требуется добавить файлы";
			return;
		}
		$ar_file = $this->base.$DIR.$ar_file;
		...
		if (sizeof($ofiles)==0) {
			...
			echo "Выберите файлы для добавления в архив";
			return;
		}
		...
		if ($ret==0) {
			...
			echo "Файлы <b>".implode(', ',$ofiles)."</b> добавлены в архив $ar_file<br>";
		}
	...
	function DownloadFile() {
		...
		$form = new JSYS_Form('DoDownloadFile',"Загрузка файла в директорию $DIR");
		$form->row('URL',"<input type=text name='url' size=60 value='http://'>");
		$form->row('Под именем',"<input type=text name='fname' value='tmp2.zip'>");
		$form->row('Способ загрузки',"<select name='dtype'><option value='1'>CURL</option></select>");
		$form->hidden('DIR',$DIR);
		$form->out();
	}
		...
		switch($dtype) {
			...
			case 1:
			{
				...
				if (!$error) {
					...
					echo "Файл $url был сохранен в $name";
				}
				else {
			...
			default:
				echo "Не известный метод загрузки файлов";
				break;
		}
	...
	function Chmod() {
		...
		if (sizeof($ofiles)==0) {
			...
			echo "Ошибка: выберите файлы для смены прав";
			return;
		}
		
		$form = new JSYS_Form('DoChmod',"Смена прав на: <b>".implode(', ',$ofiles).'</b>');
		$form->row('Права на директории',"<input type=text name='dir_mod' size=3 value='755'>");
		$form->row('Права на файлы',"<input type=text name='file_mod' size=3 value='644'>");
		$form->row('Показать лог',"<input type=checkbox name='show_log' value='1' checked>");
		$form->hidden('DIR',$DIR);
	...
	function DoChmod() {
		...
		echo "<br>Операции выполнены. <a href='jsys.php?action=FileView&DIR=".urlencode($DIR)."'>Вернуться к директории</a>";
	}
	...
	function Rename() {
		...
		if (sizeof($ofiles)==0) {
			...
			echo "Ошибка: выберите файлы для переименования";
			return;
		}
		
		$form = new JSYS_Form('DoRename','Переименование файлов и папок');
		foreach($ofiles as $file) {
	...
	function Move() {
		...
		if (sizeof($ofiles)==0) {
			...
			echo "Ошибка: выберите файлы для перемещения/копирования";
			return;
		}
		...
		$form = new JSYS_Form('DoMove',"Перемещение/копирование файлов <b>".implode(', ',$ofiles).'</b><br>Из директории: <b>'.$DIR.'</b>');
		$form->row('Действие',"<select name=what><option value='move'>Переместить</option><option value='copy' selected>Копировать</option></select>");
		$form->row('Директория',$sel);
		$form->hidden('DIR',$DIR);
		$form->hidden('ofiles',implode(',',$ofiles));
		$form->out();
	}
	...
	function grabDirs($file,$params) {
		...
		if ($cnt - $params['start_deep'] < $params['deep']) $FLIST[] = $file;//echo $file.'<br>';
	}
	...
	function DoMove() {
		...
		foreach($fnames as $fname) {
			...
			if (is_dir($from) && $act=='move') {
				...
				echo "Удаление директории $from <br>";
	...
	function recursiveEngine($file,$meth,$params) {
	    ...
	    if ($dir===false) {
	    	...
	    	echo "Не могу открыть директорию $file<br>";
	    	return;
	    }
...
class JSYSOPERATION_Info {
	...
	function main() {
		...
		echo "<p align=center>Глобальная информация</p><table align=center width=100%>";
		
		//версия пхп
		echo "<tr><td width=50%>Версия php</td><td width=50%>".phpversion()."</td></tr>";
		...
		foreach($good as $g) {
			...
			$str .= in_array($g,$e) ? "$g - <font color=green>доступно</font><br>" : "$g - <font color=red>не доступно</font><br>";
		}
		echo "<tr><td width=50%>Важные расширения php</td><td width=50%><b>$str</b></td></tr>";
		...
		if ($pos!==false) {
			...
			echo "<tr><td width=50%>Версия apache</td><td width=50%>{$m[1]}</td></tr>";
			
			if (function_exists('apache_get_modules')) {
			
				$m  = apache_get_modules();
				$mr = in_array('mod_rewrite',$m) ? 'Доступен' : 'Недоступен';
			}
			else $mr = '[не возможно определить]';

			echo "<tr><td width=50%>Доступность mod_rewrite</td><td width=50%>$mr</td></tr>";
		}
		//ось
		echo "<tr><td width=50%>Операционная система</td><td width=50%>".php_uname()."</td></tr>";
		...
		echo "<tr><td width=50%>Важная конфигурация php</td><td width=50%>$str</td></tr>";
...
class JSYS_Utils {
	...
	function link($title,$path) {
		...
		return "<a href='jsys.php?action=$action&task=$path'>$title</a>";
	}
}
class JSYS_Form {
	...
	function JSYS_Form($task,$title) {
		...
		$this->content="<p align=center>$title</p><form action='jsys.php?action=$action&task=$task' method='post'><table align=center width=80%>";
		$this->cancel = false;
	}
	...
	function out() {
		...
		$this->content.="</table><tr><td colspan=2 align=center>".($this->cancel ? "<input type=button onClick=\"window.history.go(-1);\" value='Отменить'>" : '' )."<input type=submit value='Отправить'></td></tr></form>";
		echo $this->content;
	}
...
<tr>
	...
	<font color=red>[не забудьте удалить файл после использования]</font>
	</font>
	<br><br>
	</td>
</tr>
<tr>
	...
	<a href="jsys.php?action=Main">Начало</a> <font color="#8daa94">/</font>
	<a href="jsys.php?action=SetPassword">Установить пароль</a> <font color="#8daa94">/</font>
	<a href="jsys.php?action=Info">Информация</a> <font color="#8daa94">/</font>
	<a href="jsys.php?action=FileView">Файл Менеджер</a> <font color="#8daa94">/</font> 
	<a href="jsys.php?action=Login&task=Logout">Выйти из системы</a>  
...
<tr>
	...
	<font color="#8daa94"><b>&lt;</b></font> Copyright &copy 2006 Dead Krolik <a href='http://joom.ru' target='_blank'>Joom.ru</a> <font color="#8daa94"><b>&gt;</b></font>
	</font>
</td>
</tr>
</table>
